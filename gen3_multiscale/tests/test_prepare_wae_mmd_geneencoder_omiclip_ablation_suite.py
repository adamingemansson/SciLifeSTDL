...
from gen3_multiscale.scripts import prepare_wae_mmd_geneencoder_omiclip_ablation_suite as suite_module

OMICLIP_REVISION = "d517a8dd47902dd7c308b3c36f63bce47e7b9a43"
...
def test_prepare_writes_the_four_factorial_arms_with_the_right_overrides(tmp_path):
    ...
    plan = suite_module.prepare_wae_mmd_geneencoder_omiclip_ablation_suite(
        comparison_config=str(comparison_config), manifest=str(manifest),
        train_gene_panels=str(panels), output_root=str(output_root),
        scfoundation_basis_path=str(scfoundation_basis_path), omiclip_pinned_revision=OMICLIP_REVISION,
    ...
    scf_nofilm = configs["wae_he_mmd_geneencoder_omiclip_scfoundation_nofilm"]
    ...
    mlp_film = configs["wae_he_mmd_geneencoder_omiclip_mlp_film"]
    ...
    mlp_nofilm = configs["wae_he_mmd_geneencoder_omiclip_mlp_nofilm"]
    ...
    pointer = tmp_path / "LATEST_WAE_MMD_GENEENCODER_OMICLIP_ABLATION_SUITE_ROOT.txt"
    assert pointer.read_text().strip() == str(output_root.resolve())


def test_prepare_honors_omiclip_spot_feature_cache_dir_override(tmp_path):
    comparison_config = _write_comparison_config(tmp_path)
    manifest = _write_manifest(tmp_path)
    panels = tmp_path / "panels.json"
    panels.write_text("{}")
    ...
    suite_module.prepare_wae_mmd_geneencoder_omiclip_ablation_suite(
        comparison_config=str(comparison_config), manifest=str(manifest),
        train_gene_panels=str(panels), output_root=str(output_root),
        scfoundation_basis_path=str(scfoundation_basis_path), omiclip_pinned_revision=OMICLIP_REVISION,
        omiclip_spot_feature_cache_dir="/data/custom_omiclip_cache",
    )
    config = yaml.safe_load(
        (output_root / "configs" / "wae_he_mmd_geneencoder_omiclip_mlp_nofilm.yaml").read_text()
    )
    assert config["data"]["gen3_omiclip_spot_feature_cache_dir"] == "/data/custom_omiclip_cache"
...
def test_prepare_rejects_a_blank_scfoundation_basis_path(tmp_path):
    ...
    with pytest.raises(ValueError, match="scfoundation_basis_path"):
        suite_module.prepare_wae_mmd_geneencoder_omiclip_ablation_suite(
            comparison_config=str(comparison_config), manifest=str(manifest),
            train_gene_panels=str(panels), output_root=str(tmp_path / "suite"),
            scfoundation_basis_path="", omiclip_pinned_revision=OMICLIP_REVISION,
        )


def test_prepare_rejects_a_blank_omiclip_pinned_revision(tmp_path):
    comparison_config = _write_comparison_config(tmp_path)
    manifest = _write_manifest(tmp_path)
    panels = tmp_path / "panels.json"
    panels.write_text("{}")
    ...
    with pytest.raises(ValueError, match="omiclip_pinned_revision"):
        suite_module.prepare_wae_mmd_geneencoder_omiclip_ablation_suite(
            comparison_config=str(comparison_config), manifest=str(manifest),
            train_gene_panels=str(panels), output_root=str(tmp_path / "suite"),
            scfoundation_basis_path=str(scfoundation_basis_path), omiclip_pinned_revision="",
        )
...
def test_allowed_divergent_keys_include_film_keys_only_across_a_film_boundary():
    same_film_status = suite_module._allowed_divergent_keys_between(
        "wae_he_mmd_geneencoder_omiclip_scfoundation_film", "wae_he_mmd_geneencoder_omiclip_mlp_film",
    )
    ...
    crossing_film_status = suite_module._allowed_divergent_keys_between(
        "wae_he_mmd_geneencoder_omiclip_scfoundation_film", "wae_he_mmd_geneencoder_omiclip_scfoundation_nofilm",
    )
...
def test_matched_except_declared_rejects_an_undeclared_divergence():
    ...
    with pytest.raises(ValueError, match="undeclared divergence"):
        suite_module._assert_matched_except_declared(
            control, other, "wae_he_mmd_geneencoder_omiclip_mlp_film", set(),
        )
