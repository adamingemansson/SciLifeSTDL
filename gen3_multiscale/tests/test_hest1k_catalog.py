...
def test_resolve_sample_selection_returns_patient_by_sample():
    ...
    exposed it -- needed by the real Gen3 dataset manifest, which records
    each kept sample's patient identity. Every kept sample id must appear
    with its real resolved patient value, filtered the same way
    organ_by_sample/tech_by_sample already are."""
    with tempfile.TemporaryDirectory() as tmp:
        patient_by_id = {f"L{i}": f"P{i // 3}" for i in range(9)}
        hest_dir, meta_path = _make_fake_hest1k(
            Path(tmp), {"Lung": [f"L{i}" for i in range(9)]}, patient_by_id=patient_by_id,
