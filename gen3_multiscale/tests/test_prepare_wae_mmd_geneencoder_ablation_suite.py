...
def _basis_paths(tmp_path):
    ...
    own_basis_path = tmp_path / "basis_own.pt"
    own_basis_path.write_text("stub")
    return scfoundation_basis_path, own_basis_path
...
def test_prepare_writes_the_four_factorial_arms_with_the_right_overrides(tmp_path):
    ...
    scfoundation_basis_path, own_basis_path = _basis_paths(tmp_path)
    ...
    plan = suite_module.prepare_wae_mmd_geneencoder_ablation_suite(
        ...
        scfoundation_basis_path=str(scfoundation_basis_path), own_basis_path=str(own_basis_path),
    ...
    for arm_config in configs.values():
        ...
        assert arm_config["model"]["params"]["gene_encoder_source"] == "frozen_table"
    ...
    basis_film = configs["wae_he_mmd_geneencoder_basis_film"]
    assert basis_film["model"]["params"]["encoder_conditioning"] == "film"
    assert basis_film["data"]["gene_encoder_table_path"] == str(own_basis_path)

    basis_nofilm = configs["wae_he_mmd_geneencoder_basis_nofilm"]
    assert basis_nofilm["model"]["params"]["encoder_conditioning"] == "none"
    assert basis_nofilm["data"]["gene_encoder_table_path"] == str(own_basis_path)
...
def test_prepare_rejects_a_blank_scfoundation_basis_path(tmp_path):
    ...
    panels = tmp_path / "panels.json"
    panels.write_text("{}")
    _scfoundation_basis_path, own_basis_path = _basis_paths(tmp_path)
    with pytest.raises(ValueError, match="scfoundation_basis_path"):
        suite_module.prepare_wae_mmd_geneencoder_ablation_suite(
            comparison_config=str(comparison_config), manifest=str(manifest),
            train_gene_panels=str(panels), output_root=str(tmp_path / "suite"),
            scfoundation_basis_path="", own_basis_path=str(own_basis_path),
        )


def test_prepare_rejects_a_blank_own_basis_path(tmp_path):
    comparison_config = _write_comparison_config(tmp_path)
    manifest = _write_manifest(tmp_path)
    panels = tmp_path / "panels.json"
    panels.write_text("{}")
    scfoundation_basis_path, _own_basis_path = _basis_paths(tmp_path)
    with pytest.raises(ValueError, match="own_basis_path"):
        suite_module.prepare_wae_mmd_geneencoder_ablation_suite(
            comparison_config=str(comparison_config), manifest=str(manifest),
            train_gene_panels=str(panels), output_root=str(tmp_path / "suite"),
            scfoundation_basis_path=str(scfoundation_basis_path), own_basis_path="",
        )
...
def test_allowed_divergent_keys_include_film_keys_only_across_a_film_boundary():
    same_film_status = suite_module._allowed_divergent_keys_between(
        "wae_he_mmd_geneencoder_scfoundation_film", "wae_he_mmd_geneencoder_basis_film",
    )
...
def test_matched_except_declared_rejects_an_undeclared_divergence():
    ...
    with pytest.raises(ValueError, match="undeclared divergence"):
        suite_module._assert_matched_except_declared(control, other, "wae_he_mmd_geneencoder_basis_film", set())
