...
def required_gen6_fingerprints(config: dict) -> set[str]:
    ...
    if spec.generator == "residual_ot_flow":
        required.add("gene_residual_basis")
...
def static_audit_gen6_config(config: dict) -> dict:
    ...
    expected_kind = {
        "residual_ot_flow": "flow", "wae_gan": "wae_gan",
...
def audit_gen6_manifest_cache_coverage(cache_root, sample_ids: list[str], config: dict) -> dict:
    if not sample_ids:
