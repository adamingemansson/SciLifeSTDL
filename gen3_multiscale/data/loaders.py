...
2026-07-28: copied VERBATIM from gen2_architectures/data/loaders.py at
commit 621c610 into gen3_multiscale/ -- see hest1k_catalog.py's identical
copy-provenance note in this same directory for why (the multiscale
spatial-field handoff's "reuse audited data hygiene and cache logic"
instruction). Real Gen3 data-builder work (dataset_manifest.py) needs the
exact same real-HEST-1k-loading, QC/normalization, and train-only-gene-
panel discipline this module already has audited and tested -- reusing it
here rather than reimplementing keeps both pipelines' preprocessing
contracts identical. Do NOT let this drift from gen2_architectures' copy
without a deliberate reason. If a bug is found here, check whether
gen2_architectures' copy has the same bug before fixing only one side.
