...
2026-07-28: copied VERBATIM from gen2_architectures/data/hest1k_catalog.py
at commit fd23737 into gen3_multiscale/, per the multiscale-spatial-field
architecture handoff's explicit instruction to "reuse audited data
hygiene and cache logic" rather than reimplement it. Includes the
already-audited patient-level split (split_by_patient, default True) and
cross-organ patient-conflict resolution fixed earlier this session. Do
NOT let this drift from gen2_architectures' copy without a deliberate
reason -- both pipelines currently need identical held-out-validity
guarantees. If a bug is found here, check whether gen2_architectures'
copy has the same bug before fixing only one side.
...
def _filter_held_out_ids_against_reference_panel(
    ...
    commit 9592d9e): `resolve_sample_selection` used to run
    `resolve_compatible_sample_ids` over train+validation+test IDs
    pooled together, so a validation/test sample's real panel could
