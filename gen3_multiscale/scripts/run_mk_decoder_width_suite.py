#!/usr/bin/env python3
"""Run a prepared decoder-width suite, one arm per GPU."""
...
def run_suite(root: Path, *, smoke: bool = False, dry_run: bool = False) -> dict:
    ...
    if plan.get("kind") != "mk_decoder_width_suite":
        raise ValueError("suite_plan.json is not an MK decoder-width suite")
    records = []
    ...
    try:
        ...
        for arm, process in processes.items():
            ...
            _write_status(status, status_path)
        if failures:
            raise RuntimeError(f"decoder-width arms failed: {failures}")
