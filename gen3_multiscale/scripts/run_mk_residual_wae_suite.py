#!/usr/bin/env python3
"""Run a prepared residual WAE suite, one arm per GPU."""
...
def run_suite(root: Path, *, smoke: bool = False, dry_run: bool = False) -> dict:
    ...
    if plan.get("kind") != "mk_residual_wae_suite":
        raise ValueError("suite_plan.json is not an MK residual-WAE suite")
    records = []
    for arm in plan["arm_order"]:
        ...
        records.append((arm, str(record["gpu"]), command))
    if dry_run:
    ...
    try:
        ...
        for arm, process in processes.items():
            ...
            _write_status(status, status_path)
        if failures:
            raise RuntimeError(f"residual-WAE arms failed: {failures}")
...
def main() -> None:
    ...
    print(json.dumps(run_suite(Path(args.suite_root), smoke=args.smoke,
