#!/usr/bin/env python3
"""Write two matched histology-structure-context ablation configs: the
existing decoder (opaque GigaPath context only) vs. one with the small,
zero-init-safe `HistologyContextInjector` wrapper enabled.

Both arms share IDENTICAL training hyperparameters and model dimensions
(matching wae_he_gan_control exactly) and the same task/regularizer/
include_observed_gex/dataset/masking/losses/seed/image-encoder as every
other WAE-GAN arm. Only `model.params.use_histology_context` (and its
matching `data.use_histology_features`) differs between arms -- this is
a context-injection ablation, not a training-hyperparameter,
conditioning-design, backbone, or decoder-structure ablation. Never
starts training.

Requires a histology-feature cache to already exist for every sample in
the manifest (see scripts/precompute_gen3_histology_features.py) --
neither this script nor training itself ever computes one.
"""
...
ARM_ORDER = ("wae_he_gan_histology_control", "wae_he_gan_histology")


def prepare_wae_gan_histology_ablation_suite(
    *, comparison_config: str, manifest: str, train_gene_panels: str,
    output_root: str, hours: float = 8.0, gpus: tuple[int, ...] = (0, 2),
    cpu_threads: int = 12, histology_feature_cache_dir: str | None = None,
) -> dict:
    if hours <= 0:
    ...
    for arm, gpu in zip(ARM_ORDER, gpus):
        ...
        config["experiment_name"] = f"wae_gan_histology_ablation_{arm}_v1"
        config["model"] = {
            "arm": arm,
        ...
        if arm == "wae_he_gan_histology_control":
            config["documented_divergences"] = [
                "model.arm", "training.checkpoint_dir", "training.device",
                "evaluation.tensorboard.log_dir",
            ]
        else:
            params["use_histology_context"] = True
            config["data"]["use_histology_features"] = True
            if histology_feature_cache_dir:
                config["data"]["gen3_histology_feature_cache_dir"] = str(histology_feature_cache_dir)
            config["documented_divergences"] = [
                "model.arm", "model.params.use_histology_context",
                "data.use_histology_features", "data.gen3_histology_feature_cache_dir",
    ...
    control_config = yaml.safe_load((root / "configs" / "wae_he_gan_histology_control.yaml").read_text())
    treatment_config = yaml.safe_load((root / "configs" / "wae_he_gan_histology.yaml").read_text())
    _assert_matched_except_declared(control_config, treatment_config, "wae_he_gan_histology")

    plan = {
        "kind": "wae_gan_histology_ablation_suite",
    ...
    pointer = root.parent / "LATEST_WAE_GAN_HISTOLOGY_ABLATION_SUITE_ROOT.txt"
...
_ALLOWED_DIVERGENT_KEYS = {
    ("model", "arm"),
    ("model", "params", "use_histology_context"),
    ("data", "use_histology_features"),
    ("data", "gen3_histology_feature_cache_dir"),
...
def main() -> None:
    ...
    parser.add_argument(
        "--histology-feature-cache-dir", default=None,
        help="Optional override for where the histology arm looks up its feature cache "
             "(gen3_histology_feature_cache_dir). Defaults to the same hest_cache_dir/"
             "hest_data_dir convention every other Gen3 spot-feature cache uses.",
    )
    ...
    plan = prepare_wae_gan_histology_ablation_suite(
        ...
        histology_feature_cache_dir=args.histology_feature_cache_dir,
    )
