#!/usr/bin/env python3
"""Run a prepared weak auxiliary-objective suite, one arm per GPU."""
...
def run_suite(root: Path, *, smoke: bool = False, dry_run: bool = False) -> dict:
    ...
    if plan.get("kind") != "mk_aux_objective_suite":
        raise ValueError("suite_plan.json is not an MK auxiliary-objective suite")
    records = []
    ...
    try:
        ...
        for arm, process in processes.items():
            ...
            _write_status(status, status_path)
        if failures:
            raise RuntimeError(f"auxiliary-objective arms failed: {failures}")
