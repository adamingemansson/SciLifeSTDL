...
arms -- this is a decoder-side-structure ablation, not a training-
...
def prepare_wae_gan_coexpression_ablation_suite(
    *, comparison_config: str, manifest: str, train_gene_panels: str,
    ...
    shared_params.update({
        ...
        "dropout": 0.1,
        "use_gene_coexpression_refinement": False,
    })
    ...
    for arm, gpu in zip(ARM_ORDER, gpus):
        ...
        config["experiment_name"] = f"wae_gan_coexpression_ablation_{arm}_v1"
        config["model"] = {
            "arm": arm,
        ...
        if arm == "wae_he_gan_coexpression_control":
    ...
    pointer = root.parent / "LATEST_WAE_GAN_COEXPRESSION_ABLATION_SUITE_ROOT.txt"
...
_ALLOWED_DIVERGENT_KEYS = {
    ("model", "arm"),
    ("model", "params", "use_gene_coexpression_refinement"),
    ("data", "gene_coexpression_basis_path"),
...
def main() -> None:
    ...
    plan = prepare_wae_gan_coexpression_ablation_suite(
