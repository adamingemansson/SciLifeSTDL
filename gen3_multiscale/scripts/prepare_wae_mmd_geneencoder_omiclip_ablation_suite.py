...
identical in design to `prepare_wae_mmd_geneencoder_ablation_suite.py` but
with `data.image_encoder` fixed to "omiclip" (CoCa ViT-L/14, WangGuangyuLab/
Loki) instead of "uni2". OmiCLIP's own pretraining was directly supervised
by real, paired H&E+expression contrastive alignment across 32 organs
(unlike GigaPath/UNI2's image-only pretraining), so this suite asks whether
an image tower that already "knows about" expression changes which
gene-encoder/FiLM design matters most -- still generative (ConditionalWAE,
regularizer="mmd"), only the frozen image-encoder backbone differs from the
UNI2 suite:

    arm                                              gene encoder   FiLM
    wae_he_mmd_geneencoder_omiclip_scfoundation_film   scFoundation  yes
    wae_he_mmd_geneencoder_omiclip_scfoundation_nofilm scFoundation  no
    wae_he_mmd_geneencoder_omiclip_mlp_film            plain MLP     yes
    wae_he_mmd_geneencoder_omiclip_mlp_nofilm          plain MLP     no

"wae_he_mmd_geneencoder_omiclip_mlp_nofilm" is the genuine no-intervention
control within this suite (plain encoder, no FiLM, just regularizer=mmd/
latent_dim=64, image_encoder=omiclip) -- the other three each add exactly
one real change on top of it.
...
each declared explicitly. Never starts training.
...
Requires the OmiCLIP spot-feature cache to already exist for every sample
in the manifest (see scripts/precompute_gen3_omiclip_spot_features.py).
"""
...
ARM_ORDER = (
    "wae_he_mmd_geneencoder_omiclip_scfoundation_film",
    "wae_he_mmd_geneencoder_omiclip_scfoundation_nofilm",
    "wae_he_mmd_geneencoder_omiclip_mlp_film",
...
def prepare_wae_mmd_geneencoder_omiclip_ablation_suite(
    *, comparison_config: str, manifest: str, train_gene_panels: str,
    ...
    if not omiclip_pinned_revision:
        raise ValueError("omiclip_pinned_revision must be set to the pinned WangGuangyuLab/Loki commit SHA")
    ...
    for arm, gpu in zip(ARM_ORDER, gpus):
        ...
        config["experiment_name"] = f"wae_mmd_geneencoder_omiclip_ablation_{arm}_v1"
        config["model"] = {
            "arm": arm,
    ...
    plan = {
        "kind": "wae_mmd_geneencoder_omiclip_ablation_suite",
    ...
    pointer = root.parent / "LATEST_WAE_MMD_GENEENCODER_OMICLIP_ABLATION_SUITE_ROOT.txt"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="MANDATORY: the pinned, immutable WangGuangyuLab/Loki Hugging Face commit SHA the "
             "OmiCLIP spot-feature cache was built from "
             "(scripts/precompute_gen3_omiclip_spot_features.py). Every arm in this suite uses "
             "OmiCLIP, not UNI2 or GigaPath.",
    )
    parser.add_argument(
        "--omiclip-spot-feature-cache-dir", default=None,
        help="Optional override for where every arm looks up its OmiCLIP spot-feature cache. "
