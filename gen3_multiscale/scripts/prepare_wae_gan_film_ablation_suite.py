#!/usr/bin/env python3
"""Write four matched FiLM-conditioned-encoder architecture ablation configs.

All four arms share IDENTICAL training hyperparameters and model dimensions
(matching the wae_he_gan_control arm exactly: lr=1e-4, no gradient
accumulation, full latent/hidden dims) and the same task/regularizer/
include_observed_gex/dataset/masking/losses/seed as every other WAE-GAN arm.
Only the FiLM design differs between arms -- this is an architecture
ablation, not a training-hyperparameter ablation. Never starts training.
"""
...
ARM_ORDER = (
    "wae_he_gan_film_control", "wae_he_gan_film_first_only",
    "wae_he_gan_film_last_only", "wae_he_gan_film_shared",
)

# FiLM design per arm. Training hyperparameters and model dims (latent_dim,
# autoencoder_hidden_dim, conditioner hidden_dim, discriminator_hidden_dim,
# dropout) are identical to wae_he_gan_control for every arm here -- only
# these three fields vary.
_ARM_FILM_PARAMS = {
    "wae_he_gan_film_control": {"film_layers": ["first", "second"], "film_shared_generator": False},
    "wae_he_gan_film_first_only": {"film_layers": ["first"], "film_shared_generator": False},
    "wae_he_gan_film_last_only": {"film_layers": ["second"], "film_shared_generator": False},
    "wae_he_gan_film_shared": {"film_layers": ["first", "second"], "film_shared_generator": True},
}
...
def prepare_wae_gan_film_ablation_suite(
    *, comparison_config: str, manifest: str, train_gene_panels: str,
    ...
    # dense_threshold, sparse_k, image_feature_dim: inherited verbatim, same
    # as every other WAE-GAN suite -- the FiLM generators read this same
    # context_dim regardless of arm.
    shared_params = {
    ...
    for arm, gpu in zip(ARM_ORDER, gpus):
        ...
        params.update(_ARM_FILM_PARAMS[arm])

        config = copy.deepcopy(base)
        config["experiment_name"] = f"wae_gan_film_ablation_{arm}_v1"
        config["documented_divergences"] = [
            "model.arm", "model.params.film_layers", "model.params.film_shared_generator",
            "training.checkpoint_dir", "training.device",
        ]
        config["model"] = {
            "arm": arm,
        ...
        written[arm] = {
            ...
            "audit": report,
            "gpu": gpu,
            "film_layers": _ARM_FILM_PARAMS[arm]["film_layers"],
            "film_shared_generator": _ARM_FILM_PARAMS[arm]["film_shared_generator"],
    ...
    control_config = yaml.safe_load((root / "configs" / "wae_he_gan_film_control.yaml").read_text())
    ...
    plan = {
        "kind": "wae_gan_film_ablation_suite",
    ...
    pointer = root.parent / "LATEST_WAE_GAN_FILM_ABLATION_SUITE_ROOT.txt"
