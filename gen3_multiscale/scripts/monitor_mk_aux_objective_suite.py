#!/usr/bin/env python3
"""Print one compact snapshot for an MK auxiliary-objective suite."""
...
def _rss_gib(pid) -> str:
    try:
        pages = int(Path(f"/proc/{int(pid)}/statm").read_text().split()[1])
        return f"{pages * os.sysconf('SC_PAGE_SIZE') / 2**30:.1f}GiB"
...
def main() -> None:
    ...
    for arm in plan["arm_order"]:
        ...
        print(
            f"{arm:<44} GPU={plan['arms'][arm]['gpu']} {state:<16} "
            f"PID={pid or '-'} step={max(steps) if steps else '-'} RAM={_rss_gib(pid)}"
        )
