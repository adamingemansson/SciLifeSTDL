...
from gen2_architectures.models.conditioning import (
    ...
    MLPGeneEncoder, NovaeGeneEncoder, CombinedGeneEncoder, ScFoundationGeneEncoder,
)
...
class STPathContextEncoder(nn.Module):
    def __init__(self, gene_names: list[str], gene_voc_path: str, model_weight_path: str | None = None,
        ...
        new_gene_encoder_type="scfoundation" (2026-07-25, gen2_architectures
        Architecture 4): a SEPARATE additive residual arm alongside "novae",
        not a replacement for it -- reuses the exact same _ResidualEncodeInputs
        injection mechanism and RAE pattern (frozen big representation + small
        trainable LayerNorm+Linear head, ScFoundationGeneEncoder) as "novae"
        does, just fed via its own context_scfoundation_features parameter
        (forward(), below) rather than overloading context_novae_features --
        keeps the two additive channels distinguishable if a future config
        ever wants both novae AND scfoundation residuals simultaneously,
        which nothing currently does but nothing here prevents either."""
        super().__init__()
        assert new_gene_encoder_type in ("none", "mlp", "novae", "both", "scfoundation"), (
        ...
        elif new_gene_encoder_type == "scfoundation":
            assert scfoundation_dim is not None, (
                "new_gene_encoder_type='scfoundation' requires scfoundation_dim "
                "(see precompute_scfoundation_features()'s real output shape)"
            )
            self.new_gene_encoder = ScFoundationGeneEncoder(scfoundation_dim, self.d_model)
    ...
    def forward(self, context_coords: torch.Tensor, context_expression: torch.Tensor,
                ...
                context_scfoundation_features: torch.Tensor | None = None,
        ...
        new_gene_encoder_type is set to). context_scfoundation_features:
        [N_context, scfoundation_dim] precomputed scFoundation features,
        only used/required when new_gene_encoder_type="scfoundation" — same
        additive-residual treatment as context_novae_features, kept as its
        own parameter rather than overloading that one (see __init__'s
        2026-07-25 docstring note). Returns c [N_query, hidden_dim].
        ...
        if self.new_gene_encoder is not None:
            ...
            if self.new_gene_encoder_type == "mlp":
            ...
            elif self.new_gene_encoder_type == "both":  # CombinedGeneEncoder(raw_expr, novae_features)
                assert context_novae_features is not None, (
                ...
                assert context_scfoundation_features is not None, (
                    "new_gene_encoder_type='scfoundation' requires context_scfoundation_features"
                )
                extra_embed[:n_context] = self.new_gene_encoder(context_scfoundation_features)
