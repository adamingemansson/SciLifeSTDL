...
def resolve_sample_selection(
    ...
    patient_by_sample (added for gen3_multiscale's real dataset manifest,
    applied here too to keep the two copies in sync -- see that file's
    identical note): every sample id this function actually kept -> its
    resolved patient value (see split_by_patient's own docstring above
    for exactly what that value is when a sample has no real `patient`
    metadata).
    """
    import random
