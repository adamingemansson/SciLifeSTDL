...
def cluster_pseudo_labels(
    ...
    if method == "kmeans":
        ...
        # 2026-07-27: see audit_evaluation.py::_fixed_pca's own comment --
        # measured directly on the real server that an UNCAPPED PCA.fit on
        # data this size spawns ~100 BLAS threads and burns far more CPU-
        # time in coordination overhead than the actual computation needs.
        # Capped to fully single-threaded (limits=1) rather than left
        # uncapped or capped to 8 -- a matrix this size has no realistic
        # use for multiple BLAS threads regardless.
        with threadpool_limits(limits=1):
            x = _dense_expression(adata)
            max_components = min(50, x.shape[0] - 1, x.shape[1])
            if max_components >= 2:
                x = PCA(n_components=max_components, random_state=seed, svd_solver="randomized").fit_transform(x)
