#!/usr/bin/env bash
# Four identity-preserving transport tests on st-a100 GPUs 1,2,3,5.
set -Eeuo pipefail
...
if pgrep -f -- 'src.training.train.*missing_tissue_transport_' >/dev/null; then
  echo "ERROR: gene-preserving transport processes are already running." >&2
  pgrep -af -- 'src.training.train.*missing_tissue_transport_' >&2 || true
  exit 2
fi
...
"$PYTHON_BIN" scripts/check_transport_configs.py

if [[ "$SKIP_SMOKE" != "1" ]]; then
  echo "===== Gene-preserving transport one-step smoke check ====="
  SMOKETEST=1 SMOKE_STEPS=1 STAGE=transport \
    SERVER_PROFILE=explicit_subset RUN_ID="${RUN_ID}_smoke" \
    LOG_ROOT="logs/recovery_suite/transport_${RUN_ID}_smoke" \
...
if [[ "$SMOKE_ONLY" == "1" ]]; then
  echo "Gene-preserving transport smoke check passed; full run was not started."
  exit 0
fi
...
CUDA_VISIBLE_DEVICES=1 "$PYTHON_BIN" scripts/precompute_gigapath_samples.py \
  --config configs/recovery_suite/144_transport_full_mome_k32.yaml

echo "===== Reuse leakage-safe context-only Novae graphs ====="
IFS=',' read -r -a gpu_ids <<< "$GPU_IDS_CSV"
cache_pids=()
cache_log_root="logs/recovery_suite/transport_${RUN_ID}_novae_precompute"
...
echo "===== Four identity-preserving transport full runs ====="
STAGE=transport SERVER_PROFILE=explicit_subset RUN_ID="${RUN_ID}_full" \
  LOG_ROOT="logs/recovery_suite/transport_${RUN_ID}_full" \
  GPU_IDS="$GPU_IDS_CSV" CPU_THREADS_PER_JOB="$CPU_THREADS_PER_JOB" \
  PYTHON_BIN="$PYTHON_BIN" FRESH="$FRESH" \
  bash scripts/run_recovery_suite_8gpu.sh

"$PYTHON_BIN" scripts/summarize_transport_results.py
echo "Gene-preserving transport diagnostic completed."
