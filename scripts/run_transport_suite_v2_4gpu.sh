#!/usr/bin/env bash
# hierarchical_gene_transport_regressor 20-run suite, v2 (2026-07-22 fix):
# identical ablation grid to run_transport_suite_4gpu.sh's own 165-185 suite,
# with exactly one change -- transport_reg_weight=0.0 (was 0.001). The first
# suite's own logs showed transport_head_entropy pinned near ln(128)=4.852
# (the true maximum) for the ENTIRE 20k-step run on every config: the
# neighbor-weighting mechanism never learned to specialize at all, which
# plausibly explains why every richness axis (Novae, H&E, extra heads,
# richer gates) failed to beat the raw-GEX-only baseline. See
# src/models/registry.py's HierarchicalGeneTransportRegressor.training_step
# for the code-level fix and reasoning.
#
# Same 4-capacity-gate + 16-held-out-run structure, same GPU queues, same
# preflight checks as the v1 runner -- see that script's own header for the
# full rationale (disk-space check, resumable-artifact detection, etc.).
#
...
#
# Usage:
#   bash scripts/run_transport_suite_v2_4gpu.sh
#   GPU_IDS=1,2,3,5 SMOKE_ONLY=1 bash scripts/run_transport_suite_v2_4gpu.sh
#   SKIP_SMOKE=1 bash scripts/run_transport_suite_v2_4gpu.sh
set -Eeuo pipefail
...
QUEUE_1=(186 190 194 198 202)  # O01 C01 C05 C09 C13
QUEUE_2=(187 191 195 199 203)  # O02 C02 C06 C10 C14
QUEUE_3=(188 192 196 200 204)  # O03 C03 C07 C11 C15
QUEUE_4=(189 193 197 201 205)  # O04 C04 C08 C12 C16
QUEUES=(QUEUE_1 QUEUE_2 QUEUE_3 QUEUE_4)

# CONFIG_NAME must be each config's real internal experiment_name (also its
# checkpoint_dir basename), NOT the config filename -- see the v1 runner's
# comment here for the real incident this guards against.
declare -A CONFIG_PATH
declare -A CONFIG_NAME
for f in configs/recovery_suite/18[6-9]_transport_*.yaml configs/recovery_suite/19[0-9]_transport_*.yaml configs/recovery_suite/20[0-5]_transport_*.yaml; do
...
LOG_ROOT="logs/recovery_suite/transport_suite_v2_${RUN_ID}"
REPORT_ROOT="reports/recovery_suite/transport_suite_v2_${RUN_ID}"
...
echo "===== Static fail-closed audit of all 20 transport-suite v2 configs ====="
"$PYTHON_BIN" scripts/check_transport_suite_v2_configs.py
...
if pgrep -af -- 'src.training.train.*transport_' >"$LOG_ROOT/existing_processes.txt"; then
  echo "ERROR: transport-suite jobs already exist (v1 or v2):" >&2
  cat "$LOG_ROOT/existing_processes.txt" >&2
  exit 2
fi

# Resumable-artifact detection -- see v1 runner's own comment for the exact
# reasoning; threshold is 190 here (v2's first held-out config number)
# instead of v1's 169.
job_is_done() {
  local number="$1" name="${CONFIG_NAME[$1]}"
  local ckpt="results/checkpoints/recovery_suite/${name}"
  if [[ "$number" -lt 190 ]]; then
...
run_one() {
  ...
  if [[ "$smoke" == "1" ]]; then
    ...
    if [[ "$number" -ge 190 ]]; then
      command+=(evaluation.n_validation_masks=1 evaluation.n_test_masks=1 evaluation.n_samples=1
        "evaluation.mask_bank_dir=results/mask_banks/recovery_suite/smoke_transport_v2_${RUN_ID}")
    fi
...
echo "===== Exact-mask non-learned harmonic control (same test masks as C05 v2) ====="
CUDA_VISIBLE_DEVICES="${GPUS[0]}" "$PYTHON_BIN" -m src.training.train \
  --config configs/recovery_suite/206_transport_harmonic_k128_control_v2.yaml \
  >"$LOG_ROOT/full_transport_harmonic_k128_v2.log" 2>&1 || queue_failed=1

if (( queue_failed )); then
  echo "WARNING: at least one queue/job failed or a capacity gate was not passed; inspect $LOG_ROOT/*.log" >&2
fi
"$PYTHON_BIN" scripts/summarize_transport_suite_v2.py --output "$REPORT_ROOT/summary.csv" || true
echo "Transport suite v2 run finished (RUN_ID=$RUN_ID). Report: $REPORT_ROOT/summary.csv"
echo "Logs: $LOG_ROOT"
