#!/usr/bin/env python3
"""Run one resolved marathon entry with atomic, auditable logging."""
...
from scripts.transport_marathon_lib import (
    DEFAULT_MATRIX, final_metrics_path, load_matrix, resolve_run, smoke_config,
)
...
def main() -> None:
    ...
    metrics = final_metrics_path(cfg)
    if metrics.is_file() and not args.fresh:
        print(f"SKIP: {entry.name} (completed metrics found)", flush=True)
        return

    log_root = Path(args.log_root)
    log_root.mkdir(parents=True, exist_ok=True)
    generated_dir = log_root / "generated_configs"
    generated_dir.mkdir(parents=True, exist_ok=True)
    config_path = generated_dir / f"{entry.name}.yaml"
    OmegaConf.save(cfg, config_path)
    log_path = log_root / f"{entry.name}.log"
    command = [
        args.python_bin, "-m", "src.training.train", "--config", str(config_path)
    ]
    env = os.environ.copy()
    env.update({
