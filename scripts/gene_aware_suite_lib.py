#!/usr/bin/env python3
"""Deterministic config resolution for the gated gene-aware suite."""
...
DEFAULT_MATRIX = Path("configs/recovery_suite/gene_aware_transport_16.yaml")


def load_matrix(path: str | Path = DEFAULT_MATRIX):
    matrix = OmegaConf.load(path)
    if len(matrix.get("runs", [])) != 16:
        raise ValueError(f"gene-aware matrix must contain 16 runs: {path}")
    return matrix
...
def resolve_run(matrix, index: int):
    ...
    if profile not in matrix.profiles:
        raise ValueError(f"run {entry.id} references unknown profile {profile!r}")
    base_path = Path(str(entry.base_config))
    if not base_path.is_file():
        raise FileNotFoundError(base_path)
    cfg = OmegaConf.merge(
        OmegaConf.load(base_path), matrix.profiles[profile], entry.get("overrides", {})
    )
    cfg.experiment_name = str(entry.name)
    cfg.training.seed = int(entry.seed)
    cfg.training.checkpoint_dir = f"results/checkpoints/recovery_suite/{entry.name}"
    return entry, cfg


def final_artifact_path(entry, cfg) -> Path:
    checkpoint = Path(str(cfg.training.checkpoint_dir))
    if str(entry.stage) == "overfit":
        return checkpoint / "quality_gate.json"
...
def smoke_config(cfg, run_id: str):
    ...
    cfg.training.checkpoint_dir = f"results/checkpoints/recovery_suite/smoke/{run_id}/{name}"
    ...
    smoke_root = f"results/mask_banks/recovery_suite/smoke_gene_aware/{run_id}"
    ...
    cfg.evaluation.training_mask_bank_path = (
        ...
        f"smoke_gene_aware_{run_id}_{name}.json"
    )
    return cfg
