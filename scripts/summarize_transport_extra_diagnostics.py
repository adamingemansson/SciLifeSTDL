#!/usr/bin/env python3
"""Summarize the smallhole (207-210) and local_k sweep (211-214) diagnostics.

Both are direct held-out runs (no O0X-style capacity gates), so this reuses
summarize_transport_suite_v2.py's own _heldout_row parsing of
heldout_sample_summary.json, scoped to just these 8 experiment_names --
those aren't in that script's own hardcoded HELDOUT_RUNS list, so it can't
see them.
"""
