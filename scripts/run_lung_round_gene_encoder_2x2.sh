#!/usr/bin/env bash
# Lung round, gene-encoder x architecture 2x2 (2026-07-24) -- 4
# GPU-training conditions, 1 config per GPU, smoke-tested up front
# (same pattern as run_lung_round_4gpu.sh):
#
#   GPU[0] (default 1): 306_lung_simple_stpath_transformer          (local_mlp     x SpatialTransformer)
#   GPU[1] (default 2): 307_lung_simple_cross_attn_decoder            (local_mlp     x cross-attention)
#   GPU[2] (default 3): 309_lung_spatial_transformer_universal_gene   (universal_mlp x SpatialTransformer)
#   GPU[3] (default 5): 308_lung_cross_attn_universal_gene            (universal_mlp x cross-attention)
#
# "local_mlp" = MLPGeneEncoder over this dataset's own ~12,791-gene
# ad-hoc panel (this project's usual convention). "universal_mlp" =
# the SAME MLPGeneEncoder architecture, but the input is first scattered
# into STPath's real fixed ~39k-gene-ID vocabulary space (via
# UniversalMLPGeneEncoder, src/models/simple_fusion_encoder.py) before
# encoding -- isolates whether a cross-dataset-stable gene identity space
# helps, holding encoder depth/nonlinearity fixed. Crossed with both
# "richness" architectures already built this round (SpatialTransformer
# backbone vs learned cross-attention) for a clean 2x2.
#
# 308/309 need STPATH_GENE_VOC_PATH (the same symbol2ensembl.json 301/305
# already use) to build the universal vocabulary -- unlike 306/307, which
# don't touch STPath's vocabulary at all. 306/309 need the `stpath`
# package importable (SpatialTransformer backbone); 307/308 don't.
#
# 309 shares 306's "only verified against a mocked backbone, not the
# real stpath package" caveat -- see that config's own header.
#
# Usage:
#   bash scripts/run_lung_round_gene_encoder_2x2.sh
#   GPU_IDS=1,2,3,5 SMOKE_ONLY=1 bash scripts/run_lung_round_gene_encoder_2x2.sh
#   SKIP_SMOKE=1 bash scripts/run_lung_round_gene_encoder_2x2.sh
set -Eeuo pipefail
...
if [[ -z "${STPATH_GENE_VOC_PATH:-}" || ! -f "$STPATH_GENE_VOC_PATH" ]]; then
  echo "ERROR: STPATH_GENE_VOC_PATH is missing or not a file (needed by 308/309's universal_mlp" >&2
  echo "  gene encoder -- 306/307 only via the shared config-schema check above, harmless if unused)." >&2
  exit 2
fi

echo "===== stpath package presence check (needed by 306 and 309) ====="
if ! "$PYTHON_BIN" -c 'import stpath' 2>/dev/null; then
  echo "ERROR: the external stpath package is not importable -- 306 and 309 both need it" >&2
...
echo "Lung round gene-encoder 2x2 finished: all 4 conditions completed."
